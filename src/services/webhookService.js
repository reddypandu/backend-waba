...
export class WebhookService {
      ...
      for (const change of entry.changes) {
        ...
        if (value.statuses) {
          for (const s of value.statuses) {
            ...
            const updatedMsg = await Message.findOneAndUpdate({ whatsapp_message_id: s.id }, statusUpdate, { new: true });
            ...
            if (updatedMsg && updatedMsg.requires_follow_up && (s.status === 'delivered' || s.status === 'read')) {
              await this.sendFollowUpMessage(updatedMsg, phoneNumberId, wabaId).catch(console.error);
        ...
        if (value.messages) {
          for (const msg of value.messages) {
            ...
            await this.processMessage(msg, value, phoneNumberId, wabaId).catch(e => console.error('Msg error:', e.message));
          }
        }
      }
    }
  ...
  static async processMessage(msg, value, phoneNumberId, wabaId) {
    ...
    if (content && messageType === 'text') {
      await this.checkAutoReply(userId, from, content, waAccount.phone_number_id, waAccount.access_token, conversation._id, contact._id);
    }
  }

  static async checkAutoReply(userId, to, text, phoneNumberId, accessToken, convId, contactId) {
    ...
    const r = await fetch(`https://graph.facebook.com/v24.0/${phoneNumberId}/messages`, {
      method: 'POST',
